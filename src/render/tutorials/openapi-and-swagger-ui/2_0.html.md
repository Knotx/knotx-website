...
## OpenAPI
OpenAPI is a standard specification for describing REST APIs. It allows to define endpoints, parameters, data types, constraints and metadata. This format is also the integral part of Knot.x, so it's important to know it.
...
It defines the OpenAPI specification version used and some metadata about our API (version, title, description). It then defines a list of servers our API is available on. In this case it's just one server: `localhost:8092`. Here we also provide metadata in the form of description. Note the usage of variables. They are not really necessary in this case, but let's imagine that your API is available on 3 ports and 2 domains. Instead of listing all 6 possible combinations, you can just use variables. Also, note that all variables here have to be strings and that they don't have to just be `port` and `domain`. They can be anything.
...
Secondly, we used an `example` property for the email. It's just a metadata that can be used by visualization tools (more on that later).
...
We define a list of parameters. In this case we only have one. We specify its name, type (as `query`) and metadata (description). It uses a schema in exactly the same way as request and response bodies do. New thing here is the `enum` property. It specifies that this type can only have one of the specified values. Also, note the `required: true` part. It means client can't ommit this parameter. Therefore the only two correct requests are:
- /api/user/isadmin?datatype=json
- /api/user/isadmin?datatype=xml

If a client ommits the `datatype` parameter or sets it as something other then `json` or `xml`, then Knot.x will automatically respond with `400 Bad Request`.
...
And the same schema for XML will be intepreted like this:
...
In the path, the parameters are defined using `{}` brackets. They are treated like wildcards, therefore our path will match all requests in the form of `/api/user/.../send/...`. Next, we define the same parameters in the `parameters` property, the same way as with `query` parameters. Note that the `name` in the parameter list must exactly match the name specified in the brackets. Also, path parameters have to be `required`.
...
As already mentioned, we won't discuss `header` and `cookie` parameters, because they work they work the same way as `query` parameters. Just specify `in: header` or `in: cookie`.
...
Now let's define three handlers we used above. They're really simple.

The `static-status` handler just returns a response with a given reponse code:
...
Nothing to explain here. The `static-json` handlers just reads its 'json` property from configuration, parses it to a JSON string and returns it:
...
Our application is basically working. If you run it should be able do call our endpoints. For example:
...
Most of the code here is a Swagger UI specific configuration needed for it to run. For more explanation check the [Swagger UI docs](https://github.com/swagger-api/swagger-ui). One important thing is the `url: "/swagger/spec` part. It's an url that Swagger UI will call to retrieve the OpenAPI specification file. We will expose `openapi.yaml` file under this path.

Let's add to endpoints to the `openapi.yaml` that will serve our `index.html` file and the `openapi.yaml` file itself:
...
We can see `summary` is displayed here as well. We can also clearly see that `POST /api/user` is deprecated. Let's click on the `GET /api/user/isadmin` endpoint. You should see all the information abouyt this endpoint. It consumes one required query parameter `datatype` that can either be `json` or `xml`. It can also return one of those two content types. Example responses are also provided. You can click on the `Try it out` button, specify the query parameter and `Execute`. Swagger UI will call this endpoint and show you the response. Curl commands are also provided. Swagger UI can handle many things, such as image uploading or even different types authorization. You can read more about it [here](https://swagger.io/tools/swagger-ui/).
