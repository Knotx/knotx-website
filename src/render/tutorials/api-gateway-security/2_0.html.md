...
In this tutorial, we will take a look on how to secure a API gateway.
...
paths:  
  ...
  /api/secure/basic:  // endpoint with basic auth authentication
    get:
...
As we can see, we reuse our `hellohandler` implemented in `HelloWorldHandlerFactory`, but with some different messages each time. 
...
It is worth noting that operations are not aware of the security they will be covered with.
...
In this example we'll be using Vert.x's implementation of [Apache Shiro Auth](https://shiro.apache.org/index.html). Therefore, the configuration must be in a format described [here](https://vertx.io/docs/vertx-auth-shiro/java/#_the_shiro_properties_auth_provider).

As mentioned before, we'll create the file in module's `resources` folder and name it `users.properties`.

Let's create a sample user `john` with a super secure password. He'll be an administrator with all the `role`s:
...
> Remember to stop any running containers! If there is a container that uses the `8092` port we're using, our container will not start. You can use this command to stop all running containers: `docker stop $(docker ps -a -q)`
...
1. Select `HS256` as algorithm.
2. Fill in the key you used in configuration.
...
After saving, you will be prompted to create new client credentials (if not, just open the Credentials tab on the left and a new credential).
...
Now let's implement the factories we have just defined. Let's put them in `modules/security-module/src/main/java/io/knotx/examples/security/routing` directory.
...
- `state` and `nonce` are some random string values, see [this documentation](https://developers.google.com/identity/protocols/OAuth2WebServer#creatingclient) for further details
...
4. We map the resulting observable with the token to `fetchUserInfo` whick creates a `GET` request to Google API with bearer token authentication and sends it.
