...
In this tutorial, we'll not delve into such topics as cryptography or how to store JWT inside the user's browser. We put JWT in the HTTP request headers. In the production, JWT should be stored in an httpOnly cookie, and then can be rewritten by some Apache server to headers.

Ideally, we would want to implement a flow as below:
...
Let's start with setting our secured API.
