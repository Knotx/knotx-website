...
## Overview
In this tutorial, we'll go through a process of implementing a page populated with dynamic data using [Fragments](https://github.com/Knotx/knotx-fragments) and [Fragments Chrome Extension](https://github.com/Knotx/knotx-fragments-chrome-extension). You should already know how to [setup a project](https://knotx.io/tutorials/getting-started-with-knotx-stack/2_0/) and preferably have at least basic understanding of what Fragments are (although it's not necessary). We'll focus on a relationship between Fragments and the Chrome Extension and not on setting up Fragments from the ground up.
...
This tutorial is also available as a video (TODO: link to the video).

## What Are Fragments
Before explaining what the Extension can help us with, we need to recall what Fragments are. In abstract terms, they are a mechanism for describing data flow and are used primarly to create integrations with 3rd party services. We can use them to define graphs (trees), where each node performs an action either retrieving or processing data. Those graphs are then evaluated by Knot.x.

For example, imagine a scenario where we need to display what weather is in the user's location. We have a service which can tell us about user's prefered format (e.g. Celsius or Farenheit) and another service for retrieving actual weather information based on the specified format. Fragments make this case trivial. Here's how the simplest case could look like:

![Fragments Example](/img/blog/chrome-extension/fragments-example.png)

There are three nodes defined. First for fetching user's preferred format, second for fetching weather, and third for rendering the page. When a node is finished it chooses a state (e.g. _success_, represented by the color green) and then triggers the next node based on its edges. A node with a _success_ state triggers the node defined in its `_success` edge. 

If we want to react to errors (e.g. one of the services returns code 500) we can just define additinal edges that are triggered when an action fails. All of this without writing any java code.

## What Is The Extension
The Fragments Chrome Extension is an open source tool for visualizing and debugging fragments. If you enable debug mode, the rendered HTML page (or JSON in case of a restful api) is injected with additional data about fragments procesing, that can be read by the extension. The tool displays information about:
- execution time of fragments and individual nodes inside them
- fragments' structure (in fact, the diagram above is a screenshot from the tool)
- data flow in the graph
- additinal data for individual nodes, e.g. a response a node got from a service
...
As already mentioned, we won't be going into details about how to setup the project. For this tutorial, we have an HTML page like this:

![Raw Page](/img/blog/chrome-extension/raw-page.png)

As you can see, right now it's not even processed. There is a raw templating engine markup rendered. In code, it's just a plain HTML with two Knot.x snippets inside. One of them contains markup for User Details section, and the other for Payment Providers and Offers sections. It's worth noting that while the first snippet uses [Handlebars](https://handlebarsjs.com/), the second one uses [Pebble](https://pebbletemplates.io/) templating engine. It's possible for multiple markups to exist in the same HTML file, because snippets are processed independently from each other.
...
We defined `user-task` as an object that represents a root node of the graph. Right now, this node contains a single property `action` that defines what action this node is supposed to call. Think of actions as behaviours. 
...
The most important property to explain is the `factory` piece. This defines which java factory should produce the action. Think of it as of a type of the action. In this case we use the `http` factory (a factory registered under the name "http"). It's a built-in action that calls a specified address and retrieves data from it. The `config.endpointOptions` property should be self explanatory. In case you're wondering why the domain is defined as `webapi` - this example runs as a docker swarm configured so that mocks are available at this host. If you're not using docker, it'd be localhost.

Note that you can also write and register your own factories using java, but it's not in the scope of this tutorial.

If we were to leave the configuration like this we would create a REST API. We want to render a page though, so we need another node in our task. When the first node succeeds (so user data is fetched), we want to call a node that renders a page. This is the updated code for the `user-task`:
...
In this case we define that after a node with action `fetch-user-info` Knot.x should call a node with action `te-hbs`. This action will render the page using handlebars. This is how it looks like:
...
Perhabs not what you were expecting. The `knot` factory allows for registering actions on the [Vert.x Event Bus](https://vertx.io/docs/vertx-core/java/#event_bus), thus providing more scalability. Here's how the actual handlebars configuration looks like:
...
There are two fragments because we have two snippets in HTML. We haven't defined a task for the second one thus it displayes a grey square next to it. It means it wasn't processed. Ignore it for now and focus on the first one. It's green so it ended with a success. There's also a randomly generated ID, a type ("snippet" is the only possible fragment type in an HTML page), and the time it took to process the fragment. If you click on the arrow pointing down next to the ID, you should see a list of top level HTML elements in the snippet. In this case it's just one div. Hover over it to highlight the div on the page.
...
We defined four nodes that execute sequentialy one after the other. The first one reuses already defined action called `fetch-user-info`. There is also `fetch-payment-providers` and `fetch-offers` actions that we haven't defined yet. Finally, at the end there is a node with `te-pebble` action. This snippet uses pebble templating engine so we have to define a new action for rendering it. If it used handlebars we could reuse our `te-hbs` action.
...
It works, but it's not the best approach. Right now the `fetch-offers` node waits for the `fetch-payment-providers` to succeed. There is no reason for that (remmeber, we assumed that those pieces of information can be retrieve intependently). There would also be a problem if `fetch-payment-providers` failed for some reason. Then `fetch-offers` would not execute, even if it would be able to retrieve data and show them to the user. It'd be more efficient and safer if those two nodes could execute in parallel.

There are two types of nodes. Everything you've seen up to this point were so called "single nodes". They just execute an action. The second type is a composite node. This kind of node contains an array of other nodes. It executes them in parallel and waits for all of them to complete. If all of them succeed then the composite succeeds. Otherwise the composite fails. This is how we can reorder our nodes to make use of a composite:
...
This is a good moment to introduce the perfomance tab. When you open the tab called "Perfomance View" you'll see a timeline of node execution:

![Timeline](/img/blog/chrome-extension/timeline.png)

By looking at this it's clear that `fetch-payment-providers` and `fetch-offers` executed simultanously and that the next node (`te-pebble`) executed only after both of them finished.
...
When we load the page now it should take about three seconds. If we open the Knot.x extension we can immediatly spot on the fragment list that the second fragment executed for a long time. When we open the fragments performance view we can clearly see that the problem lays in the composite and specificaly in the `fetch-offers` node.

What if such a long time is unacceptable? One approach would be to define timeout in the `fetch-offers` action. However, we can do better. Knot.x provides us with a built-in implementation of a [Circuit Breaker Pattern](https://en.wikipedia.org/wiki/Circuit_breaker_design_pattern). It does so by a [Circuit Breaker Factory](TODO-find-a-link-to-documentation) that acts as a wrapper. Let's see how it can be configured:
...
Firstly, we call `fetch-offers-cb` instead of `fetch-offers` in the composite. That's because the `fetch-offers` action will be called inside of `fetch-offers-cb`. Also, since we expect the task to fail, the composite should handle the `_error` transition. It will be called if any subtask ends with anythink different then success. In this case we still want to render the page so we call the `te-pebble` action. 
...
The yellow node with an exclamation mark is a so called "missing node". It doesn't actually exist during processing. It's just displayed in the tool as a way to catch the developer's attention. It means that a transition was triggered (in this case `_fallback`) but there was no node to handle it. Because of that, the `_fallback` state was returned to the composite and thus the composite ended with an error (composites can only end with either `_success` or `_error`).
...
The `fetch-offers-fallback` action could retrieve data from cache or from another service. In our case it'll just call the `/offers` endpoint:
...
The only problem left is that we can't see any offers displayed, even though they were retrieved (you can check they were by viewing details of the fallback node). Let's take a look at the HTML snippet:
