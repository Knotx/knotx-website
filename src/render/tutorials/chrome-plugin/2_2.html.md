...
As already mentioned, we won't be going into details about how to set up the project. For this tutorial, we have an HTML page like this:
...
Note that you can also write and register your own factories, but it's not in the scope of this tutorial.
...
Perhaps not what you were expecting. The `knot` factory allows for registering actions on the [Vert.x Event Bus](https://vertx.io/docs/vertx-core/java/#event_bus), thus providing more scalability. Here's how the actual handlebars configuration looks like:
...
There are two types of nodes. Everything you've seen up to this point was so-called "single nodes". They just execute an action. The second type is a composite node. This kind of node contains an array of other nodes. It executes them in parallel and waits for all of them to complete. If all of them succeed then the composite succeeds. Otherwise, the composite fails. This is how we can reorder our nodes to make use of a composite:
...
This is a good moment to introduce the performance tab. When you open the tab called "Performance View" you'll see a timeline of node execution:
