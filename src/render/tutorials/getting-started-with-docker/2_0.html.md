...
- How to set up a Knot.x project based on the [Knot.x Starter Kit](https://github.com/Knotx/knotx-starter-kit) template using Docker
...
It's a [OpenAPI 3.0](https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.0.md) specification file that defines the configuration of our project.
...
Create a new module under `modules` folder. The structure should look like this:
...
If you take a closer look at build log (you can use `-i` flag for a more verbose process) you can see, that not only our project is being build, but also it:
