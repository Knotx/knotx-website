...
- How to setup a Knot.x project based on the [Knot.x Starter Kit](https://github.com/Knotx/knotx-starter-kit) template using Docker
- How to create a simple Handler and route it to specific path
...
It's a [OpenAPI 3.0](https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.0.md) specification file that defines configuration of our project.
...
We can see three distinguishable parts of this file:
 ...
 3.  Paths configuration that explicitly specifies our endpoints, their paths, methods, responses and much more in a clear, easily readable and declarative way:
      ...
      ...
      ```
      Our starter-kit template is comes with some already defined paths out of the box. Let's take the `/healthcheck` above as an example. This endpoint serves purpose of testing whether our dockerized distribution is up and running.
      
      We can clearly see that it is registered under `/healthcheck` path, it's method is a `GET` and it can have various responses depending on the state of the instance.
      
      There's no magic behind it. Healthcheck is handled by operation (more on that later in this tutorial) `healthcheck-operation` implemented as a handler in `modules/health-check/src/main/java/com/project/example/healthcheck/HealthcheckHandlerFactory.java`. Feel free to take a look at it's implementation, but we'll not focus on it right now. Just to be brief: it simply calls our example endpoints (defined in `knotx/conf/openapi.yaml` as well). 
      
      This is a really nice feature to have! Since we will be using Docker in this tutorial, we will tell docker to check if our instance is up and running by simply calling this endpoint and expecting a response.
...
Now that we know what's going on in this file, let's define our own endpoint.
...
Now let's take a closer look on what lies under `operationId: hello-world-operation` ...
...
Now we'll have to provide some handler to handle our request. Let's put it in a separate module.

Create new module under `modules` folder. The structure should look like this:
...
The most important thing is to register it by it's name. In our case it was `"hellohandler"`. 
...
If you take a closer look at build log (you can use `-i` flag for more verbose process) you can see, that not only our project is being build, but also it:
