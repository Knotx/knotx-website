...
- How to create a simple [Handler](https://github.com/Knotx/knotx-server-http#knotx-http-server) and configure it in the `/api/hello` processing chain
...
This is a HOCON (*Human-Optimized Config Object Notation*) file. More on HOCON  can be found in [this informal specification](https://github.com/lightbend/config/blob/master/HOCON.md).

Here we use [OpenAPI 3.0](https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.0.md) specification file that exposes APIs.
...
We can see three distinguishable parts of this file:
 ...
 2.  Server configuration that defines how our server behaves:
      ...
      ...
      ```
      Here we can see, for example, that our server will be registered on port `8092`. Notice variable substitution indicated by `{`brackets`}`. For more information on what can be configured here, see [this documentation](https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.0.md#serverObject). 
 3.  Paths configuration that explicitly specifies our endpoints, their paths, methods, responses and much more in a clear, easily readable and declarative way:
      ...
      ...
      ```
      Our Starter Kit template comes with example paths and methods defined. Let's see the `/healthcheck` API. It is used to express the current state of our application in very simple terms: UP or DOWN (more on that later in this tutorial).
      ...
      There's no magic behind it. Healthcheck is handled by a uniquely named operation. In this case: `healthcheck-operation`. We will take a look at how they are connected later in this tutorial. 
      
Now that, we know what's going on in this file, let's define our custom endpoint `/api/hello` and operation `hello-world-operation`.
...
As previously mentioned, paths are connected with [operations](https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.2.md#operationObject). Definition of handlers involved in the processing of a request for a specific operation is done in form of [Routing operations](https://github.com/Knotx/knotx-server-http/tree/master#routing-operations). They are defined in `knotx/conf/routes/operations.conf`. 
...
`${routingOperations}` takes care of merging any previously defined routing operations with the ones you define here. More on that can be found [here](https://github.com/lightbend/config/blob/master/HOCON.md#array-and-object-concatenation).
...
Now that, we have a feeling of it, let's connect our `hello-world-operation` with a handler. Let's name the handler `hellohandler` (note: names must be unique!). We'll also make our message configurable.
...
Now we'll have to provide a handler to handle the request. Let's put it in a separate module.
...
But what *building the Docker image* means in our case?
...
The first test is the `healthcheck`. Technically speaking, this is not a *test*, but rather an endpoint in which, we call our own endpoints within our application. If the request succeeds, we can be sure that our Docker container is up and running.
...
...
```
Docker is trying to reach `localhost:8092/healthcheck` with defined interval, timeouts and retries. If `/healthcheck` returns `200` code, we can be sure that our instance is up. For more details see [this documentation](https://docs.docker.com/engine/reference/builder/#healthcheck).
