...
Create `<KNOTX-STARTERKIT-ROOT>/common-services` directory. Let's put this directory in the main directory of project, since we will be reusing files created there in different tutorials. Now let's create `<KNOTX-STARTERKIT-ROOT>/common-services/webapi/__files` and put 4 files there:
