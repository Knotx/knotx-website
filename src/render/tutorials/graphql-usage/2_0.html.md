...
## GraphQL
GraphQL is a protocol, a way of serving and retrieving data. Unlike REST, GraphQL uses a single endpoint that is serving all data. Clients specify what data they need with a dedicated query language. The server then responds with JSON including only data the client asked for and in the structure he or she wanted.
...
The main idea of this library are the **data fetchers**. A data fetcher is an object implementing the `DataFetcher` interface with only one method: 
...
Save it as a `books.graphqls` file in your app resources.
...
Our fetcher will parse configuration to create tasks and run them using [`FragmentsEngine`](https://github.com/Knotx/knotx-fragments/tree/master/handler/engine). It will output a `Fragment` that will contain data fetched from google in its payload. Fragment's payload is just a `JsonObject` where actions can store data.
