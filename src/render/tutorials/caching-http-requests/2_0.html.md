...
## Actions & behaviours
The first thing we are going to do is to define our custom task which will properly handle our request. As you probably know,
...
The most important for us here is `/product/id` endpoint definition. As you can see we are connecting this endpoint with
`product-api-caching-proxy-operation` which we've defined before. Moreover here we've provided some other specification 
but it was described in other tutorials, so if you don't feel familiar with it, please check them out.
