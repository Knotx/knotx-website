...
To sum it up in this tutorial we've configured in-memory cache and show how easily we can replace it with another one.
What's important we did it without writing any line of code, but only using Knot.x configs. We've focused on creating it
in declarative way instead of implementing cache on our own. Hopefully, now you'll know how easily you can extend other 
