...
- How to wrap an existing (external) API with caching proxy
...
Knot.x tasks are directed graphs of actions (if you want to get more familiar with configurable integrations please read [this](https://knotx.io/blog/configurable-integrations/) tutorial). 
...
Below task definition you can see all necessary actions definitions. As mentioned before, we've created `fetch-product-with-cache`. The purpose 
...
from the server for every request - we can cache and reuse the response in the future (if not changed) - it results in a quicker
...
You may also wonder if we can replace this cache with another one, for example with [Redis](https://redis.io/) - yes you can do it very simple!
...
to process the request. Now we'll focus on operation called `product-api-caching-proxy-operation` (it will be connected to `openapi.yaml`).
