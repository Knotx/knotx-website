...
Now if you invoke Knot.x at [`localhost:8092/product/id`](localhost:8092/product/id) and refresh it a few times, you can see that neither the timestamp nor the random string in the JSON response changes - we have successfully cached external API response!
