...
Download [Knot.x distribution](https://knotx.io/download) and unzip it.
...
To do so, open `conf/includes/serviceKnot.conf` and add the `bookslist` service:
```hocon
...
```
Now, as we have the `bookslist` datasource, let's edit the `conf/includes/serviceAdapter.conf` file and configure Adapter available under `knotx.adapter.service.http`.
