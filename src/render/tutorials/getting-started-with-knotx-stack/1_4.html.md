...
- How to transform static HTML into dynamic content and configure Knot.x to use simple REST endpoint to get data
- How to use data from such endpoint to dynamically populate HTML
...
To do so, open `conf/includes/dataBridge.conf` and add `bookslist` data definition:
```hocon
# Event bus settings
...
```
Now, as we have `bookslist` data definition, let's edit `conf/includes/dataSourceHttp.conf` file and configure Adapter available
